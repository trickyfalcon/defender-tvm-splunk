#!/usr/bin/env python3
"""Defender TVM BULK fetcher (Memory Optimized).

Fetches ALL current vulnerabilities across all machines.
Use for initial baseline and periodic full refreshes.
...
def build_event(record: Dict[str, Any], catalog_entry: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Build CIM-compliant event from bulk record."""
    ...
    event = {
        ...
        "recommended_update": record.get("recommendedSecurityUpdate"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Defender TVM Bulk Fetcher")
    ...
    sys.stderr.write("Fetching ALL vulnerabilities via bulk endpoint...\n")
    bulk_url = f"{API_BASE}/machines/SoftwareVulnerabilitiesByMachine"

    event_count = 0
    severity_counts: Dict[str, int] = {}

    for record in paged_get_streaming(
        bulk_url, headers, verify,
        limit=args.limit if args.limit > 0 else None,
        max_retries=args.max_retries,
        continue_on_error=args.continue_on_error,
        request_delay=args.api_sleep,
    ):
        severity = record.get("vulnerabilitySeverityLevel", "Unknown")
    ...
    # Summary
    sys.stderr.write(f"Retrieved {event_count} vulnerability records.\n")
    for severity, count in sorted(severity_counts.items()):
        sys.stderr.write(f"  {severity}: {count}\n")
